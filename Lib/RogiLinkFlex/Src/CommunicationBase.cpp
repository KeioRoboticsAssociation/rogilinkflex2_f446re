...
uint8_t* _global_header_removed_buffer_ptr;
...
// 受信時の処理
void CommunicationBase::on_receive(uint8_t frame_id, size_t size) {
    uint8_t* buffer = _global_header_removed_buffer_ptr;
...
void CobsEncodedCommunicationBase::on_receive_raw(uint8_t* data, uint8_t size) {
    ...
    if (!error) {
        _global_header_removed_buffer_ptr = header_removed_buffer;
        on_receive(id, size - 2);
    }
}
